...
SOURCE = ROOT / "artifacts/chronos/gfe_axial_221_epsilon0_root_refinement_certificate.json"
source_bytes = SOURCE.read_bytes()
source = json.loads(source_bytes)
assert source["schema"] == "chronos.gfe_axial_221_epsilon0_root_refinement_certificate.v1"
assert source["scope"]["ell"] == 2 and source["scope"]["overtone"] == 1
...
record={
  "schema":"chronos.gfe_polar_221_gr_darboux_certificate.v1",
  "date":"2026-09-25",
  "status":"certified_gr_epsilon0_polar_221_root_via_exact_darboux_transfer",
  "scope":{"parity":"polar","ell":2,"m":2,"overtone":1,"epsilon":0.0,"dimensionless_frequency":"Omega=M*omega","branch":"Schwarzschild GR baseline inherited from certified axial 221 root refinement","beta2_polar_correction":False,"unreduced_trace_log_spectrum":False},
  ...
  "claim_boundary":["This certificate proves only the Schwarzschild GR polar (ell,m,n)=(2,2,1) root at epsilon=0 by exact Darboux transfer from the certified axial root.","It does not derive the relative O(beta^2) polar quadratic action, polar master potential, polar root continuation, or polar frequency derivative.","It does not certify axial-polar splitting for epsilon>0.","The unreduced trace-log spectrum and its additional massive mode remain outside this certificate."]
}
print("GFE_POLAR_221_GR_DARBOUX")
...
print("RESULT := CERTIFIED_POLAR_221_GR_ROOT_TRANSFER")
