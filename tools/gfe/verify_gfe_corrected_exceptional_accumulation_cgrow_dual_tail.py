...
def main() -> None:
    ...
    minimum_gap_at_R = min((item[3] for item in gap_data), key=lambda x: float(sp.N(x, 30)))
    if sp.simplify(minimum_gap_at_R > 0) is not sp.true:
        raise AssertionError("minimum growing-channel tail gap is not positive")

    # Banach fixed-point estimate for q=e_g+Kq with ||K||<=mass.
    ...
    # does not affect nonvanishing, and the dual enclosure yields the rigorous
    # finite-radius test |e_g^T Z|-deviation*||Z||_1 > 0.
    ...
    print(f"VOLERRA_MASS_BOUND := {sp.sstr(mass)} <= 1/16")
    print(f"GROWING_MODEL_MIN_REAL_GAP_AT_TAIL := {sp.sstr(minimum_gap_at_R)}")
    print("GROWING_DUAL_VOLERRA_KERNEL := modulus <= 1 for all five complementary channels")
    print("GROWING_DUAL_EXISTENCE := unique normalized adjoint row q_g with q_g(infinity)=e_g^T by contraction mapping")
    print(f"GROWING_DUAL_DEVIATION_BOUND := ||q_g-e_g^T||_inf <= {sp.sstr(deviation)} <= 1/15")
    print("CGROW_FINITE_RADIUS_IDENTITY := C_grow=exp(-phi_g(R))*q_g(R)*Z_phys(R)")
    print("CGROW_NONVANISHING_TEST := |Z_grow(R)| > deviation*||Z_phys(R)||_1 is sufficient")
