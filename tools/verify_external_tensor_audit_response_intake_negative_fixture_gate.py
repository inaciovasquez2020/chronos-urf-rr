...
FIXTURE_PATH = ROOT / "fixtures/chronos/external_tensor_audit_response_invalid_forbidden_signal_fixture_2026_06_10.json"
ARTIFACT_PATH = ROOT / "artifacts/chronos/external_tensor_audit_response_intake_negative_fixture_gate_2026_06_10.json"
...
def main() -> None:
    ...
    assert artifact["id"] == "EXTERNAL_TENSOR_AUDIT_RESPONSE_INTAKE_NEGATIVE_FIXTURE_GATE_2026_06_10"
    assert artifact["date"] == "2026-06-10"
    assert artifact["repository"] == "chronos-urf-rr"
    assert artifact["status"] == "VERIFIER_ONLY"
    assert artifact["object"] == "EXTERNAL_TENSOR_AUDIT_RESPONSE_INTAKE_NEGATIVE_FIXTURE_GATE"
    ...
    assert artifact["expected_rejection_reason"] == "FORBIDDEN_SIGNAL_PRESENT"
    assert artifact["closed_object"] == "EXTERNAL_TENSOR_AUDIT_RESPONSE_INTAKE_NEGATIVE_FIXTURE_GATE"
    ...
    assert fixture["response_id"] == "SYNTHETIC_INVALID_FORBIDDEN_SIGNAL_RESPONSE_FIXTURE_2026_06_10"
    assert fixture["auditor_identity"]["kind"] == "synthetic_fixture"
    assert fixture["audit_target"] == schema["id"]
    assert fixture["checked_identity"] == schema["route"]
    assert fixture["verdict"] == "VALID"
    assert "RESPONSE_SUPPLIED" in boundary_tokens(fixture)

    assert rejection_reason(schema, fixture) == artifact["expected_rejection_reason"]

    print("EXTERNAL_TENSOR_AUDIT_RESPONSE_INTAKE_NEGATIVE_FIXTURE_GATE_OK")
