...
ARTIFACT = Path(
    "artifacts/chronos/"
    "restricted_continuation_normal_trace_existence_discharge_lemma_2026_06_15.json"
)

EXPECTED_SOLVES = "RESTRICTED_CONTINUATION_NORMAL_TRACE_EXISTENCE_DISCHARGE_LEMMA"
EXPECTED_PREDECESSOR = "RESTRICTED_CONTINUATION_BOUNDARY_TRACE_EXISTENCE_DISCHARGE_LEMMA"
EXPECTED_NEXT = "RESTRICTED_CONTINUATION_TRACE_PAIRING_DISCHARGE_LEMMA"


def main() -> None:
    data = json.loads(ARTIFACT.read_text())

    assert data["artifact"] == "restricted_continuation_normal_trace_existence_discharge_lemma"
    ...
    assert lemma["name"] == "restricted_continuation_normal_trace_existence_discharge_lemma"
    assert "normal trace" in lemma["claim"]
    assert "flux object" in lemma["claim"]
    assert lemma["domain"] == "restricted continuation region"
    assert lemma["boundary"] == "partial Sigma_R"
    assert lemma["normal_trace_object"] == "restricted_continuation_normal_trace"
    ...
    assert "flux_field_in_divergence_trace_domain" in names
    assert "divergence_regularitiy_on_restricted_region" in names
    assert "normal_trace_operator_available_on_restricted_region" in names
    assert "restriction_compatible_with_normal_boundary_chart" in names
    ...
    assert "Does not identify the normal trace with a physical flux pairing." in non_claims
    ...
    print("RUNALL_RESTRICTED_CONTINUATION_NORMAL_TRACE_EXISTENCE_DISCHARGE_LEMMA_OK")
