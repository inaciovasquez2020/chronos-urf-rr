...
artifact = json.loads((ROOT / "artifacts/chronos/genuine_four_pde_component_existence_reduction_2026_05_22.json").read_text())
...
assert "import Chronos.Frontier.GenuineFourPDEComponentExistenceReduction" in chronos
assert "import Chronos.Frontier.GenuinePDEEstimateWitnessComponents" in lean
...
assert "theorem genuinePDEEstimateWitnessComponentsExist_from_four_components" in lean
assert "def GenuineFourPDEComponentExistenceReductionTarget : Prop" in lean
assert "theorem genuinePDEEstimateWitnessComponentsExist_from_four_component_target" in lean
assert "axiom " not in lean

assert artifact["status"] == "FOUR_COMPONENT_EXISTENCE_REDUCTION_CLOSED_COMPONENT_PROOFS_OPEN"
assert artifact["next_admissible_object"] == "GENUINE_FOUR_PDE_COMPONENT_EXISTENCE_REDUCTION_TARGET"
...
print("Genuine four PDE component existence reduction verification OK.")
