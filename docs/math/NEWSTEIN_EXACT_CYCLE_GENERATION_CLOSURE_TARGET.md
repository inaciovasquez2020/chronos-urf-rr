...
Let \(L\) be the rooted local complex and let \(T\subseteq L\) be a spanning tree.

Then for every non-tree edge \(e\in E(L)\setminus E(T)\), the unique tree path in \(T\) joining the endpoints of \(e\) defines a fundamental cycle
\[
C_e := e + P_T(e).
\]
