...
/--
Bridge the bounded finite-jet exclusion component surface into the analytic
Green-kernel tail construction assumptions.

This reuses the certified estimate-input bridge.  It does not prove the
finite-jet exclusion estimate or the full Green-kernel estimates internally.
-/
def certified_uniform_finite_jet_exclusion_estimate_component_surface_to_construction_assumptions
    {X : DFMSIDFHFieldSpace}
    {S : DFMSIDFHSpectralProbe X}
    (I : NonlocalRenormalizedLogDet S)
    (x : X.State)
...
/--
Bridge the bounded obstruction-estimate component surface into the analytic
Green-kernel tail construction assumptions.

This reuses the certified estimate-input bridge.  It does not prove the
obstruction estimate, finite-jet exclusion estimate, or full Green-kernel
estimates internally.
-/
def certified_uniform_obstruction_estimate_component_surface_to_construction_assumptions
    {X : DFMSIDFHFieldSpace}
    {S : DFMSIDFHSpectralProbe X}
    (I : NonlocalRenormalizedLogDet S)
    (x : X.State)
...
/-- Expose the weakest currently named analytic Green-kernel obligation directly
from the full component aggregate surface.

This is only the composition of the aggregate-to-estimate-input bridge with the
estimate-input obligation projection. It does not prove the analytic obligation
or the full Green-kernel estimates internally.
-/
def certified_uniform_full_green_kernel_aggregate_surface_weakest_missing_obligation
    {X : DFMSIDFHFieldSpace}
    {S : DFMSIDFHSpectralProbe X}
    (I : NonlocalRenormalizedLogDet S)
    (x : X.State)
