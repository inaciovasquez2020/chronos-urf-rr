...
def test_horizon_special_frequency_n2_after_forced_n1_log_pair() -> None:
    """Test n=2 solvability after the n=2 compatibility fixes c1 and d1."""
    data = json.loads(ARTIFACT.read_text())
    M, beta, lam, omega, r = _symbols()
    ...
    c1 = 5 * a0 * beta * (lam - 2) / (12 * M * (5 * M**2 + beta))
    d1 = -2 * M * c1
    ...
    variables = (a2, b2, c2, d2)
    matrix, rhs = sp.linear_eq_to_matrix(n2_conditions, variables)
    solution = sp.linsolve((matrix, rhs), variables)
    assert solution != sp.EmptySet, f"forced-pair n=2 system inconsistent: {n2_conditions}"
