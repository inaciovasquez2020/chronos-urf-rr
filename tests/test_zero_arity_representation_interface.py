...
def test_zero_arity_representation_interface_artifact():
    data = json.loads((ROOT / "artifacts/chronos/zero_arity_representation_interface.json").read_text())
    ...
    assert "ZeroArityExclusion remains an admissible alternative" in data["alternative_route"]
    assert "no ZeroArityRepresentation proof or downstream theorem-level closure asserted" in data["boundary"]

def test_zero_arity_representation_interface_doc_boundaries():
    text = (ROOT / "docs/status/CHRONOS_ZERO_ARITY_REPRESENTATION_INTERFACE_2026_05_09.md").read_text()
